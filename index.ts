...
export function getGithubToken(): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    iframe.src = `https://gh-authtoken-test.onrender.com/github/login?appUrl=${encodeURIComponent(appUrl)}`;

    const handleMessage = (event: MessageEvent) => {
      if (typeof event.data === 'string' && event.data.startsWith('gh-token:')) {
